...
class Database:
    ...
    def get_data(self, query_type, start_date, end_date, schema='well_day_histories'):
        ...
        if column:
            self.cursor.execute(f'''
                SELECT {data}, {column} FROM {schema}
...
class MainWindow(QWidget):
    ...
    def update_results(self):
        ...
        data_fact = self.db.get_data(selected_query, start_date, end_date, schema='well_day_histories')

        # Получаем данные из базы данных для well_day_plans
        data_plan = self.db.get_data(selected_query, start_date, end_date, schema='well_day_plans')
